#!/usr/bin/env python3
"""REHEARSAL-ONLY version bump (V8-003 §5). Applies the 8.0.0 identity to every surface the release gates read, inside a
...
sh("git", "add", "-A"); sh("git", "-c", "user.name=YuClawLab", "-c", "user.email=vzhang2099@gmail.com", "commit", "-q", "-m", f"release(v{NEW}) candidate: version {NEW} on every gate-checked surface (badge re-pinned on the shared-header pages; README transcript from the release-candidate wheel; PyPI description regenerated) — PREPARED IN PRIVATE STAGING FOR OWNER REVIEW; not applied to the branch")
