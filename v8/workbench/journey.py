...
class Journey:
    ...
    def demo_features(self, page, a: str, b: str, A, B, wsA):
        ...
        page.goto(f"{a}/claim/{CID}"); ssec = self.section(page, "sci"); csec_now = self.section(page, "calculation")
        self.check(F, "the claim page lists the linked scientific record with its status and input identity, states that the link yields no scientific improvement about the commitment, and the claim's own result is unchanged", sid_linked in ssec and "SUPPORTED_REPLAY" in ssec and "no scientific improvement about the commitment" in ssec and "Overall: IN_RANGE" in csec_now, ssec[:200])
