...
<form class="card" method="post" action="/verify" enctype="multipart/form-data">{self._csrf_field()}<label>Export zip</label><input type="file" name="packet"><button type="submit">Verify</button></form>{res_html}"""
    ...
    def page_help(self, q) -> str:
        ...
        links = [("/", "Workspace overview — claims, computed results, fictional fixtures"), ("/source", "1 Source — register a passage or an ingestion record; as-of view"), ("/source#availability", "1 Source — correct a wrong availability time with a linked event (the registration is never edited)"), ("/claim/new", "2 Typed claim — save and freeze a commitment"),
                 ...
                 ("/modules", "Modules — SHD, EVO, COM and PRC: what each does, its status and its limits"), ("/setup", "Setup and status — local principals and the restricted worker's live probe"), ("/login", "Sign in (needed on every page once a principal exists)"),
                 ("/shd", "SHD Distillation Shield — submit a bundle, request a decision"), ("/shd/trust", "SHD — trust roots, policy and approvals (administrator)"), ("/evo", "EVO Evolution Evidence Audit — versions, configuration, requests"),
                 ("/com", "COM Research Commons Guard — the review queue, budgets, disputes"), ("/prc", "PRC Independent Practice — tasks, sessions, follow-ups, checkpoints"), ("/modx", "Module evidence export and verification (use a fresh workspace to verify)")]
