...
Representative output below was produced by running the INSTALLED rehearsal wheel (built from candidate commit `cd6103c300c4613d0fd0ccc8a14fcfa0fc59e00f`; NOT the final artifact) in a directory outside any checkout, with an empty environment and synthetic stores. Exit-code contracts: `packet` 0 SUCCESS / 1 MISMATCH / 2 usage / 3 UNSUPPORTED; `receipts`, `challenge`, `decision` 0 ok / 1 contract, authority or input error / 2 usage / 3 (history) unusable snapshot. A synthetic walkthrough never becomes a real outsider receipt; a packet SUCCESS is not an outsider result, not evidence of official origin and not a scientific validation.
...
[packet] built <work>/cwd/pk: 10 files, source cd6103c300c4; verify with: yuclaw packet verify <work>/cwd/pk
[exit 0]
```
```
$ yuclaw packet verify ./pk
[packet] SUCCESS — manifest 493a6d2b42755c17…; exact bytes verified and published Lab statistics/ledger roots reproduced from the frozen bundle; not an outsider receipt, not a research interpretation, not proof of official origin | provenance: official artifact equality UNVERIFIED
[exit 0]
```
```
$ yuclaw packet verify ./pk --trusted-manifest trusted.json   # identity obtained through an independent channel
[packet] SUCCESS — manifest 493a6d2b42755c17…; exact bytes verified and published Lab statistics/ledger roots reproduced from the frozen bundle; not an outsider receipt, not a research interpretation, not proof of official origin | provenance: official artifact equality EQUAL
[exit 0]
```
```
$ yuclaw packet verify ./pk   # after one byte of an artifact changed
[packet] MISMATCH — manifest 493a6d2b42755c17…; first discrepancy: byte mismatch at docs/capabilities.json: expected 96beff4fa51b4f91…/3341 B, observed 266555f98b64662f…/3341 B | provenance: official artifact equality UNVERIFIED
[exit 1]
```
## Journey 3 — create and evaluate a challenge

Limits: a challenge states a criterion; a `general` criterion is resolved only by a designated reviewer's explicit evaluation; verifier-backed criteria need the store's own executed verification record for the REVISED artifact; dispositions never erase the original finding. The reviewer credential comes from `--token-file` (mode 0600), never from an argument. (Here the challenged artifact is a synthetic earlier bundle identity and the revised artifact is the packet's real bundle.)

```
$ yuclaw challenge --store ./store --synthetic create ch-1 --artifact-type bundle --sha256 63793dced306… --size-bytes 30 --claim-id claim-1 --expected "replay exit 0" --observed "replay exit 1" --criterion packet-integrity-and-replay
{
 "challenge_id": "ch-1",
...
{
 "verification_id": "7e04a9fc9ba768b744f50ff83b5e996a67e7dfbd46a0bc3c9213d2734fffd5a8",
 "challenge_id": "ch-1",
 "criterion": "packet-integrity-and-replay",
 "method": "packet-verify",
 "result": "SUCCESS",
 "reason": "packet verification SUCCESS and the revised artifact's exact bytes were observed in it",
 "executed_at": "2026-09-14T07:50:44.895549Z"
}
[exit 0]
```
```
$ yuclaw challenge --store ./store --synthetic dispose ch-1 RESOLVED --role rev-syn --token-file tok --revised-type bundle --revised-sha256 <sha> --revised-size-bytes <n> --revised-path ./pk/artifacts/docs/replay/lab_replay_bundle.json --allowed-root . --verification-id <id>
{
 ...
 "version": 2,
 "resolution.verification_method": "packet-verify",
 "resolution.criterion": "packet-integrity-and-replay"
}
[exit 0]
```
```
...
[
 {
  ...
  "disposition": "RESOLVED",
  "adverse": false,
  "disposed_by_authority": "SYNTHETIC"
 }
]
[exit 0]
```
```
$ yuclaw challenge --store ./store --synthetic history ch-1   # every version retained; the original finding is never erased
[{"version": 1, "disposition": "OPEN"}, {"version": 2, "disposition": "RESOLVED"}]
[exit 0]
```
...
```
$ yuclaw decision --store ./store --synthetic record dec-1 REQUEST_EVIDENCE --packet-manifest-digest 493a6d2b4275… --claim-id claim-1 --context '{"note": "private context"}'
{
 "decision_id": "dec-1",
...
Limits: exact-target coverage is UNBOUND until a release target manifest is delivered as release evidence; `explain` shows reason codes publicly and private facts only with `--private`; `history` compares two checked public snapshots (synthetic boards are refused) and never reports a multiplier from a zero baseline.

```
$ yuclaw receipts --store ./store --synthetic explain demo-1
{
 "receipt_id": "r-81bdf995baa4bc93b68f1a008ce7fd9a",
 "attempt_id": "demo-1",
...
```
```
$ yuclaw receipts --store ./store --synthetic --now 2026-09-14T08:00:00.000000Z scoreboard --out a.json   # add --target <release target manifest> to bind exact-target coverage
