...
- `yuclaw receipts --store <private> …` imports an attempt, verifies artifact bytes, records a designated review and derives counts. Primary counts are **qualified attempts** (failed and inconclusive retained); successful attempts are reported separately; package evidence (wheel, sdist) is separate from site checks; program-wide exact-artifact evidence is separate from a release's exact-target evidence (bound to an independently supplied target manifest, UNBOUND until then). A submission can describe an outcome; it cannot award itself qualification. `yuclaw receipts --store <private> explain <receipt id>` answers "why did this count / not count?" with reason codes; `yuclaw receipts --store <private> history a.json b.json` compares two published scoreboard snapshots (additions, supersessions rendered CORRECTED in their original window, disposition changes, per-window and absolute movement).
